...
def evaluate(sentence):
    ...
    encoded_sentence = np.expand_dims(encoded_sentence, axis=0)

    output = model.predict([encoded_sentence, np.zeros((1, 1))])

    return output[0]

def decode(encoded_sentence):
    decoded_sentence = []
    for word_index in encoded_sentence:
        if word_index == 0:
            break
        decoded_sentence.append(vocab[word_index])
    return ' '.join(decoded_sentence)

for question in test_questions:
    encoded_question = encode(preprocess_sentence(question))
    encoded_question = np.expand_
...
encoder_model = tf.keras.Model(inputs, encoder_states)

decoder_state_input_h = tf.keras.Input(shape=(decoder_units,))
decoder_state_input_c = tf.keras.Input(shape=(decoder_units,))
